...
func TestDialKeyspaceNotCreated(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("Unexpected nil when connecting to database with not created keyspace")
	}
}
...
func TestHappyPath(t *testing.T) {
	...
	t.Run("Reading data", func(t *testing.T) {
		...
		for i, v := range []byte("Testing") {
			if v != data.Data[i] {
				t.Fatalf("Data mismatch from previously set data and data just read, got %+v expected %+v", data.Data[i], v)
			}
		}
	})
